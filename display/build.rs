...
fn waveshare() {
    // This is the directory where the `c` library is located.
    let libdir_path = PathBuf::from("waveshare")
        // Canonicalize the path as `rustc-link-search` requires an absolute
        // path.
        .canonicalize()
        .expect("cannot canonicalize path");
    ...
    // This is the path to the `c` headers file.
    let headers_path = libdir_path.join("DEV_Config.h");
    let headers_path_str = headers_path.to_str().expect("Path is not a valid string");

    // Tell cargo to look for shared libraries in the specified directory
    println!("cargo:rustc-link-search={}", libdir_path.to_str().unwrap());

    // Tell cargo to tell rustc to link our `DEV_Config` library. Cargo will
    // automatically know it must look for a `DEV_Config.a` file.
    println!("cargo:rustc-link-lib=DEV_Config");
    ...
    // Write the bindings to the $OUT_DIR/bindings.rs file.
    let out_path = PathBuf::from(env::var("OUT_DIR").unwrap()).join("DEV_Config_bindings.rs");
    bindings
        .write_to_file(out_path)
        .expect("Couldn't write bindings!");
}
